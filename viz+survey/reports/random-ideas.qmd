...
p6 <- ggplot(apipop) + geom_point(aes(y = api00, x = avg.ed), alpha=1/50 ) + theme(aspect.ratio = 1)+ labs(title='population', x = '', y = '')

# ggplot(apipop) + geom_point(aes(y = api00, x = avg.ed, color=stype), alpha=.1 ) + 
#  facet_grid(~stype) + 
...
apistrat.exp <- apistrat |> uncount( round(wts) ) |> 
  mutate(api00 = api00+rnorm(n(), 0, 20), 
  avg.ed=avg.ed+rnorm(n(), 0, .2), 
  growth = growth +rnorm(n(), 0, 12) )

p5 <- ggplot(apistrat.exp) + geom_point(aes(y = api00, x = avg.ed), alpha=1/50) +
   theme(aspect.ratio = 1)+ labs(title='Exp sample', x = '', y = '') 
...
p6 <- ggplot(apipop) + geom_point(aes(y = growth, x = avg.ed), alpha=1/50 ) + 
  theme(aspect.ratio = 1) + labs(title = 'population', x = '', y = '')

# p5 <- ggplot(apistrat) + geom_point(aes(y = growth, x = avg.ed) ) +
#   theme(aspect.ratio = 1) + labs(title='sample', x = '', y = '') 

p5 <- ggplot(apistrat.exp) + geom_point(aes(y = growth, x = avg.ed), alpha=1/50) +
   theme(aspect.ratio = 1)+ labs(title='Exp sample', x = '', y = '') 
...
- Big discussion on weather use weights in linear models, exogenous sample vs endogenous sample.

Endogenous sample, R (selected or not in the sample) is independent of $Y$ conditional on a set of $X$ variables


- Usual methods: fit model with and wothout using weights, compare fits (with a test?) and decide . NOT SURE IF THEY (EC.) CHECK THIS
